...
'''
  ...
  # TODO: bad idea
  # Find up line
  up_line = (down_line[0], down_line[1] + 150 - height // 2, down_line[2],
             down_line[3] + 150 - height // 2)  # start_width start_heigh end_width end_heigh
...
def Hand_line_detection(img):
  ...
  def on_closing():
    if messagebox.askokcancel("Wrong way((",
                              "Reminder! Line coordinates are not transferred when the program is closed. Do you want to quit? "):
      win.destroy()
  ...
  def Rotate(event, arg):
    ...
    Centers[arg] = (((math.fabs(Line_Starts[arg][0] - Line_Starts[arg][2])) / 2),
                    ((math.fabs(Line_Starts[arg][1] - Line_Starts[arg][3])) / 2))
...
def image_homography(img):
  ...
  # View image
  cv2.imshow('img', cv2.resize(img, (800, 450)))
  cv2.imshow('bird_view', cv2.resize(bird_view, (1720, 880)))
  # cv2.imshow('cropped_image', cv2.resize(cropped_image, (1920, 1080)))
  return bird_view


if __name__ == '__main__':
  img = cv2.imread('../resources/dataset/BirdView/011---taiyuan/west_2.jpg')
